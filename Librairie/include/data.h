 ...
 * Projet      : Fusée
 * Fichier     : data.h
 * Description : API echanges de donnees (envoi, reception, dialogue).
 *
...
/**
 *	\fn			void dialogueClient(socket_t *sockEch, const char *msg, char *reponse, int repSize, ...)
 *	\brief		Dialogue client simple (envoi puis reception)
 *	\param 		sockEch : socket d'échange
 *	\param 		msg : message à envoyer
 *	\param 		reponse : buffer de reception
 *	\param 		repSize : taille du buffer de reception
 *	\note		En mode DGRAM, fournir en plus l'adresse IP et le port du serveur.
 */
void dialogueClient(socket_t *sockEch, const char *msg, char *reponse, int repSize, ...);
/**
 *	\fn			void dialogueServeur(socket_t *sockEch, char *msgRecu, int msgSize, const char *rep)
 *	\brief		Dialogue serveur simple (reception puis envoi)
 *	\param 		sockEch : socket d'échange
 *	\param 		msgRecu : buffer pour le message recu
 *	\param 		msgSize : taille du buffer de reception
 *	\param 		rep : reponse a envoyer
 */
